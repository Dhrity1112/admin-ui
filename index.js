...
all_members = [...members];
// populatedField(members)
// createPaginationButtons();
preservedHtml = document.getElementsByTagName("table")[0].innerHTML;
all_members.length <= 0 && (document.getElementsByTagName("table")[0].innerHTML = `<b>No Data Returned by API</b>`)
createPaginationButtons(all_members);
all_members.length > 0 && updateTable(1, all_members);
})
.catch((error) => {
...
document.getElementsByTagName("table")[0].innerHTML = `<b>No Data Matching Search Criteria Present</b>`;
} else {
document.getElementsByTagName("table")[0].innerHTML = preservedHtml;
tbody = document.getElementsByTagName("tbody")[0];
} //else document.getElementsByTagName("table")[0].innerHTML = preservedHtml;
...
const endIndex = Number(Object.keys(deleteMap)[Object.keys(deleteMap).length - 1])

// const new_all_members = [all_members.slice(0, startIndex), all_members.slice(endIndex + 1, all_members.length - 1)]
const new_all_members = [...all_members.slice(0, startIndex), ...all_members.slice(endIndex + 1, all_members.length)]
